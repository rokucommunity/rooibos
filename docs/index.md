...
| preExitSleepTime?                  | number          | How long (in milliseconds) to sleep before exiting the app when `keepAppOpen` is false, to give the IO/telnet connection time to finish sending all the logs. The default is 400                    |
