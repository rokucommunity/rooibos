        ...
        describe('transpilation in setup hooks', () => {
            it('transpiles stubCall inside beforeEach', async () => {
                program.setFile('source/test.spec.bs', `
                    ...
                    class Klass
                        function getFunction(arg1 = invalid)
                        end function
                    end class
                `);
                program.validate();
...
function getFunctionContents(rawCode: string, functionNameRegex: RegExp) {
    ...
    if (funcStmt) {
        const start = funcStmt.func.body.location.range.start;
        const end = funcStmt.func.body.location.range.end;
...
function expectFunctionContents(fileContents: string, functionName: string, expectedContents: string) {
    const contents = getFunctionContents(fileContents, new RegExp(`^${functionName}$`));
    if (contents) {
