...
sub RBS_STATS_AppendSuiteStatistic(statTotalObj as object, statSuiteObj as object)
 ...
 sub RBS_TR_Run()
  ...
  RBS_TR_RunItGroups(metaTestSuite, totalStatObj, m.testUtilsDecoratorMethodName, m.config, m.runtimeConfig, m.nodeContext)
    end if
    skipSuite:
  end for
  m.logger.PrintStatistic(totalStatObj)
  if  RBS_CMN_IsFunction(RBS_TR_ReportCodeCoverage)
  RBS_TR_ReportCodeCoverage()
  end if
  RBS_TR_SendHomeKeypress()
end sub
 sub RBS_TR_RunItGroups(metaTestSuite, totalStatObj, testUtilsDecoratorMethodName, config, runtimeConfig, nodeContext = invalid)
  ...
  RBS_TR_RunTestCases(metaTestSuite, itGroup, testSuite, totalStatObj, config, runtimeConfig)
 ...
 sub RBS_TR_RunTestCases(metaTestSuite, itGroup, testSuite, totalStatObj, config, runtimeConfig)
...
function RBS_TR_Rooibos_RunNodeTests(args) as object
  ? " RUNNING NODE TESTS"
  totalStatObj =  RBS_STATS_CreateTotalStatistic()
  RBS_TR_RunItGroups(args.metaTestSuite, totalStatObj, args.testUtilsDecoratorMethodName, args.config, args.runtimeConfig, m)
  return totalStatObj
end function
function RBS_TR_Rooibos_CreateTestNode(nodeType) as object
    ...
    return invalid
      ...
      TestRunner_instance = {
        __className: "TestRunner"
         __TestRunner: RBS_TR_new
        Run: RBS_TR_Run
        RunItGroups: RBS_TR_RunItGroups
        RunTestCases: RBS_TR_RunTestCases
        SendHomeKeypress: RBS_TR_SendHomeKeypress
      }
